...
def extract_run_id(filepath: str) -> str:
    ...
    if len(parts) >= 3:
        ...
        run_id = '_'.join(parts[-1].split('.')[0].split('_'))
...
def generate_markdown_report(results: List[Dict], run_id: str, output_dir: str):
    ...
    with open(report_path, 'w') as f:
        ...
        for model_name in model_names:
            ...
            f.write(f"| {model_name} | {m['em']}/{total} | {m['em_pct']:.1f}% | {m['avg_f1']:.3f} | {m['avg_time']:.2f} | {m['avg_calls']:.1f} |\n")
        ...
        for model_name in model_names[1:]:
            wins = sum(1 for r in results if r['models'][model_name]['f1'] > r['models'][baseline]['f1'])
            losses = sum(1 for r in results if r['models'][model_name]['f1'] < r['models'][baseline]['f1'])
            ties = sum(1 for r in results if r['models'][model_name]['f1'] == r['models'][baseline]['f1'])
        ...
        f.write(f"- **Most Efficient (Fewest Calls):** {efficient_model} ({metrics[efficient_model]['avg_calls']:.1f} calls avg)\n")
        
        f.write("\n---\n\n")
        
        # Sample Comparisons
        f.write("## Sample Comparisons (First 5)\n\n")
        ...
        for i, r in enumerate(results[:5], 1):
            ...
            for model_name in model_names:
                ...
                f.write(f"| {model_name} | {answer_preview} | {m['f1']:.2f} | {'✓' if m['em'] else '✗'} | {m['time']:.1f}s | {m['llm_calls']} |\n")
...
def print_console_summary(results: List[Dict]):
    ...
    for model_name in model_names:
        ...
        print(f"{model_name:<30} | {em_count}/{total} ({em_count/total:.0%}){'':<2} | {avg_f1:.3f}    | {avg_time:.2f}s      | {avg_calls:.1f}")
